...
module.exports = function WebService(verbose) {
  ...
  this.get = function (route, handler) {
    ...
    route = new RegExp("^" + route + "$", "mi");
    this.__routes.push({ method: "GET", route: route, handlers: handlers });
  };
  ...
  this.del = function (route, handler) {
    ...
    this.__routes.push({ method: "DELETE", route: route, handlers: handlers });
  };
  ...
  this.post = function (route, handler) {
    ...
    if (!this.__routes) { this.__routes = []; }
    route = new RegExp("^" + route + "$", "i");
    this.__routes.push({ method: "POST", route: route, handlers: handlers });
  };
  this.put = function (route, handler) {
    ...
    if (!this.__routes) { this.__routes = []; }
    route = new RegExp("^" + route + "$", "i");
    this.__routes.push({ method: "PUT", route: route, handlers: handlers });
  };
      ...
      req.on("end", function () {
        ...
        function determineRoute() {
          for (var index = 0; index < self.__routes.length; index += 1) {
            ...
            if ((matches = req.url.split('?')[0].match(route.route)) &&
              req.method.match(route.method, "i")) {
              req.matches = matches.length && matches.splice(1);
              req.matches = req.matches.filter(function (match) {
                return !(match && match.toString().indexOf("?_=") > -1);
              });
